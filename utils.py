...
指令4: 查询jj (目标牛牛长度, 自己或者他人, 通过艾特选择对象, 没有at时目标是自己)
...
指令9: 淫趴介绍 | 淫趴说明| 淫趴帮助 (输出淫趴插件的命令列表)"""
        ...
        with open(config_file, 'r', encoding='gbk') as f: # 获取配置
            config = json.loads(f.read())
        self.dj_cd_time: int = config["djcdtime"]    # 打胶冷却时间
        self.pk_cd_time: int = config["pkcdtime"]     # pk冷却时间
        self.suo_cd_time: int = config["suocdtime"]     # 嗦牛子冷却时间
        self.fuck_cd_time: int = config["fuckcdtime"]    # 透群友冷却时间
